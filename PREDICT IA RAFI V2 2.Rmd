---
title: "SCRIPT R-STUDIO PREDICT-IA V2"
---
...
type_doc=doc$TYPE_DOC#liste des noms de documents correspodant aux comptes rendus d'hospitalisation 
rm(doc,type_doc)
```

## Définir les dates de l etude
...
```{r setup, include=FALSE}
#Parametre de rmakdown (evite les messages inutiles)
...
## Collecter compte rendus des services 
 ...
 depart=depart-1#évite decalage des dates 
 arrivee=arrivee-1
...
###Formater les dates des urgences et encoder mois/jour
...
Filter patients hospitalises apres passage aux urgences
```{r}
...
Séparer les blocs de textes des urgences (IAO, anamnese, ATCD ...) et creer dataframe avec textes decoupés
...
### Variables binaires des urgences
...
Denombrement des variables binaires 
Rationnel: le nombre d'ATCD, detraitements en cours, de symptomes et d'exammens complémentaires est probablement un indicateur utile pour prédire la survenue d'évenements critiques . Cette operation correpond à une reduction de dimension
...
###Jointure des dataframes binaires 
Objectifs: 
-disposer d'un seul dataframe ("df_bin") avec toutes les var binaires explicatives (issues des urgences: CRH urgences, labo, imagerie) 
-disposer d'un seul dataframe ("df outcome") avec toutes les var binaires expliquées
(donnees post urgences: hospit et PMSI)

4 étapes: 
-1 joindre les multiples dataframe binaires des urgences entre
eux (ATCD, motif de venue, symptomes...)\
-2 reduire les dataframe labo , image, hospit et PMSI à 1 ligne par
ID-SEJ 
-3 joindre les df des var explicatives (urgences + labo +
imagerie) pour former df_bin 
-4 joindre les df des var expliquées
(hospit +PMSI) pour former df_outcomes

Etape 1 :Jointure des dataframes binaires des urgences
...
Exemple: si 1 séjour aux urgences (AVC) entraine une hospitalisation dans 2
services (neurologie puis medecine polyvalente) , il y a 2 lignes dans le df
...
```{r}
  #intubation en service pour detresse respiratoire sans intubation aux urgences.
  
  df_outcome$intub_hosp_dyspnee=NA
  df_outcome$intub_hosp_dyspnee=ifelse(
...
Objectif: créer 2 dataframes  sur
lesquels porteront les analyses 
-df_outcome comportera les variables
expliquées 
-df_final comportera les variables explicatives
...
##Reduction nombre variables 
Retirer les variables tres peu collectées par les cliniciens. 
...
##### Intubation critique en hospitalisation

###### Intubation en hospitalisation 
Cela est retrouvé dans le PMSI
...
###### Dyspnée ou coma en hospitalisation

l'outcome "intubation" de notre étude doit etre associé a la presence de'un coma ou
d'une dyspnée car sinon il englobe les intubation au bloc
operatoire (ex: fracture avant bras).
...
#####Arret cardiaque en hospitalisation 
...
Remplacer ancien df_outcome par df_outcome final et charger dataframes a analyser
...
#Reduction dimension du dataframe 
regroupement par classes therapeutiques 

## Reduction dimension ttt_habituels
...
#MODELISATION 
faite avec exploring tidymodels (kaggle) et le package "probably" de max kuhn
...
##Choix de l'outcome
A choisir dans la variable "noms_outcome"
...
Etapes 
-créer un dataframe comprenant l'outcome
...
#Imputation df_train
  ...
  names=str_replace(names, "ximp.", "")#enleve les suffixes ximp. rajoutés lors de l'implémentation
  ...
  names=names(df_train_num_imput)#enleve les suffixes ximp. rajoutés lors de l'implémentation
...
##PR-AUC
pour chaque fold pour chaque seuil
...
## Déterminer le meilleur seuil (threshold)
...
##Compraraison pourcentages  et moyennes
